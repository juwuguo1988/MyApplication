...
import com.clj.fastble.thread.BtMessagePool;
...
public class BleManager {
    ...
    private BtMessagePool mBtMessagePool;
    ...
    public void init(Application app) {
        if (context == null && app != null) {
            ...
            mBtMessagePool = new BtMessagePool();
            mBtMessagePool.btStart();
        }
    }

    public BtMessagePool getmBtMessagePool() {
        return mBtMessagePool;
    }
    ...
    public void write(BleDevice bleDevice,
        ...
        if (bleBluetooth == null) {
            ...
            createData(dataInfoQueue, bleBluetooth, uuid_service, uuid_write, operatorType, callback);
        }
    }

    private synchronized void createData(Queue<byte[]> dataInfoQueue, BleBluetooth bleBluetooth, String uuid_service,
                                         String uuid_write,
                                         int operatorType,
                                         BleWriteCallback callback) {
        for (byte[] data : dataInfoQueue) {
            BleHandler handler = new BleHandler(bleBluetooth, data, uuid_service, uuid_write, operatorType, callback, true);
            BleManager.getInstance().getmBtMessagePool().setQueue(handler);
        }

//        for (int i = 0; i < 6; i++) {
//            BleHandler handler = new BleHandler(bleBluetooth, null, uuid_service, uuid_write, operatorType, callback, false);
//            BleManager.getInstance().getmBtMessagePool().setQueue(handler);
//        }
