...
import com.example.hello.myapplication.utils.FileUtils;
import com.example.hello.myapplication.utils.cardphoto.camera.IDCardCamera;
import com.example.hello.myapplication.utils.cardphoto.cropper.CameraPreview;
import com.example.hello.myapplication.utils.cardphoto.cropper.CropImageView;
import com.example.hello.myapplication.utils.cardphoto.listener.CropViewFinishListener;
import com.example.hello.myapplication.utils.device.DeviceUtils;
...
import com.example.hello.myapplication.utils.ui.UIUtils;
 ...
 * Desc	        ${拍照界面}
 */
public class TakeUserCardPhotoActivity extends Activity implements View.OnClickListener {
    ...
    private void init() {
        setContentView(R.layout.activity_take_user_card_photo);
    ...
    private void initView() {
        ...
        float screenMinSize = Math.min(XZLApplication.horizontalScreenWidth, XZLApplication.horizontalScreenHeight);
        float screenMaxSize = Math.max(XZLApplication.horizontalScreenWidth, XZLApplication.horizontalScreenHeight);
    ...
    public void onClick(View v) {
        ...
        if (id == R.id.camera_preview) {
            ...
            if (DeviceUtils.hasFlash(this)) {
                ...
                Toast.makeText(this, R.string.user_photo_no_flash, Toast.LENGTH_SHORT).show();
            }
        } else if (id == R.id.tv_camera_result_ok) {
            confirm();
    ...
    private void takePhoto() {
        ...
        DeviceUtils.getCamera().setOneShotPreviewCallback(new Camera.PreviewCallback() {
            @Override
            public void onPreviewFrame(final byte[] bytes, Camera camera) {
                ...
                new Thread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        Bitmap bitmap = BitmapUtils.getBitmapFromByte(bytes, w, h);
                        cropImage(bitmap);
                    }
                }).start();
            }
    ...
    private void setTakePhotoLayout() {
        ...
        mViewCameraCropBottom.setText(getString(R.string.user_touch_to_focus));
    ...
    private void confirm() {
        /*手动裁剪图片*/
        mCropImageView.crop(new CropViewFinishListener() {
            @Override
            public void onCropViewFinish(Bitmap bitmap) {
                if (bitmap == null) {
                    Toast.makeText(getApplicationContext(), getString(R.string.user_photo_crop_fail), Toast.LENGTH_SHORT).show();
                    finish();
                }

                /*保存图片到sdcard并返回图片路径*/
                String tmpDir = Environment.getExternalStorageDirectory().toString() + "/XZLFile/Photo/";
                if (FileUtils.createOrExistsDir(tmpDir)) {
                    StringBuffer buffer = new StringBuffer();
                    String imagePath = "";
                    if (mType == IDCardCamera.TYPE_IDCARD_FRONT) {
                        imagePath = buffer.append(tmpDir).append("ID_CARD_PHOTO").append(".").append("idCardFrontCrop.jpg").toString();
                    } else if (mType == IDCardCamera.TYPE_IDCARD_BACK) {
                        imagePath = buffer.append(tmpDir).append("ID_CARD_PHOTO").append(".").append("idCardBackCrop.jpg").toString();
                    }

                    if (BitmapUtils.save(bitmap, imagePath, Bitmap.CompressFormat.JPEG)) {
