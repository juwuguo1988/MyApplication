...
//        drawGrid(canvas);//裁剪框内部线条
    ...
    public void crop(CropViewFinishListener cropListener, boolean needStretch) {
        ...
        if (cropRect.width() <= 0 || cropRect.height() <= 0) { //用户裁剪的宽或高为0
            cropListener.onCropViewFinish(null);
            return;
        }
        ...
        if (!needStretch) {
            cropListener.onCropViewFinish(cut);
        } else {
...
//            stretchCanvas.drawBitmap(cut, matrix, null);
            ...
            cropListener.onCropViewFinish(stretch);
        }
    }
